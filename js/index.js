...
$(document).ready(function () {
    ...
    $("head").append("<link href='https://fonts.googleapis.com/css?family=Roboto:300,400,700,900&amp;subset=cyrillic-ext' rel='stylesheet' type='text/css'>");
    ...
    document.querySelector('.scc .table-prev').addEventListener('click', function () {
        table.prev()
    });
    document.querySelector('.scc .table-next').addEventListener('click', function () {
        table.next()
    });
    ...
    document.querySelector('.scc .prev').addEventListener('click', function () {
        mySiemar.prev()
    });
    document.querySelector('.scc .next').addEventListener('click', function () {
        mySiemar.next()
    });
});
//
...
//работает только на сервере с CDN
let gallery = document.querySelector('.gallery'),
    galleryItems = gallery.querySelectorAll('.gallery-item'),
    galleryPopup = document.querySelector('.gallery-popup'),
    popupItem = galleryPopup.querySelector('.popup-item'),
    closeGallery = document.querySelector('.mfp-close'),
    nextImg = document.querySelector('.mfp-arrow-right'),
    prevImg = document.querySelector('.mfp-arrow-left'),
    img = document.createElement('img');

for (let i = 0; i < galleryItems.length; i++){
    galleryItems[i].addEventListener('click', function () {
        let source = galleryItems[i].querySelector('img').src;
        source = source.slice(0, source.lastIndexOf('/') + 1);
        img.src = source + 'gallery' + galleryItems[i].querySelector('img').getAttribute('data-img') + '.jpg';
        console.log(img.src);
        popupItem.appendChild(img);
...
galleryPopup.addEventListener('click', function (e) {
    let target = e.target;
    if(target === closeGallery || target !== popupItem.querySelector('img') && target != nextImg && target != prevImg) {
        galleryPopup.classList.remove('active');
    }
});

nextImg.addEventListener('click', function () {
    let popupSrc = popupItem.querySelector('img').src;
    for(let i = 0; i < galleryItems.length; i++){
        let source = galleryItems[i].querySelector('img').src;
        source = source.slice(0, source.lastIndexOf('/') + 1);
        if(popupSrc === galleryItems[i].querySelector('img').src) {
            let index = Number(galleryItems[i].querySelector('img').getAttribute('data-img')) + 1;
            if(index > 4) index = 1;
            popupItem.querySelector('img').src = source + 'gallery' + index + '.jpg';
        }
    }
});
        ...
        function initiate() {
            ...
            function checkPosition(selector, container, screenHeight) {
                ...
                if (contHeight > screenHeight) {
                    ...
                    container.style.margin = top - 40 + 'px auto 20px';
...
(function dayTimer() {
    var different = $('body').hasClass('timer-different');

    function iteration(){

        var hours = $('.d_hours'), minutes = $('.d_minutes'),
            seconds = $('.d_seconds'), dayHours = 0, dayMinutes = 0,
            daySeconds = 0, d = new Date();

        dayHours = 23 - d.getHours();
        dayMinutes = dayMinutes + 59 - d.getMinutes();
        daySeconds = daySeconds + 60- d.getSeconds();
        ...
        function differentChange(element, time){
            time = time + '';
            var timeArr = time.split('');

            element.each(function(i){
                if ( i % 2 === 0 ) {
                    $(element[i]).text(timeArr[0]);
                }else{
                    $(element[i]).text(timeArr[1]);
                }
            });
        }

        if (different) {
            differentChange(hours, dayHours);
            differentChange(minutes, dayMinutes);
            differentChange(seconds, daySeconds);

        }else{
            hours.text(dayHours);
            minutes.text(dayMinutes);
            seconds.text(daySeconds);
        }
